/**********************************************
* File: GoogleQueue.cpp
* Author: Matthew Morrison
* Email: matt.morrison@nd.edu
* 
* This is a solution to the Google License Interview question 
* using std::queue to store the dictionary words
* 
* Compilation command: g++ -g -std=c++11 -Wpedantic GoogleQueue.cpp -o GoogleQueue
* Run: ./GoogleQueue [DictionaryFile] [License]
* Example: ./GoogleQueue words.txt RT011OS
...
*
* Comparison to forward_list - Give the different order and answer (alphabetical)
* Less memory overhead due to pointers. Needs push and pop 
**********************************************/
#include <iostream>
#include <fstream>
...
* Pre-conditions : std::ifstream& dictionaryFile, std::queue< std::string >& dictionaryQueue
* Post-conditions: none
*  
* This function reads all the dictionary words, and stores them in a STL Queue 
********************************************/
void getDictionaryWords(std::ifstream& dictionaryFile, std::queue< std::string >& dictionaryQueue){
	// Get the strings and push in the Queue
	std::string wordIn;
	while (dictionaryFile >> wordIn)
	{
		dictionaryQueue.push(wordIn);
	}
}
...
void checkPlate(int argc, char* fileName, char* plateStr){

	// Create a Queue of dictionary words 
	std::queue< std::string > dictionaryQueue;
	...
	// Get the strings and put in the Queue
	getDictionaryWords(dictionaryFile, dictionaryQueue);
	...
	while(!dictionaryQueue.empty()){
		
		// Check the word, and update final word if it qualifies
		// Do not hash dictionaryQueue.top() if final Word is the same length - Save run time 
		// Must also account for if no word is found yet (finalWord.length() > 0)
		if(dictionaryQueue.front().length() < finalWord.length() || finalWord.length() == 0){
		
			// Check the word, and update final word if it qualifies
			HashWord(HashPlate, dictionaryQueue.front(), finalWord);
...
int main(int argc, char** argv){
	...
	checkPlate(argc, argv[1], (char*)"RCT100SA");
	checkPlate(argc, argv[1], (char*)"RT123SO");
	checkPlate(argc, argv[1], (char*)"AQ10S0K");
	checkPlate(argc, argv[1], (char*)"TNT055RB");
	checkPlate(argc, argv[1], (char*)"LET10EE0");
	checkPlate(argc, argv[1], (char*)"AB1C1D1");
	checkPlate(argc, argv[1], (char*)"AEI1O2U3");
	checkPlate(argc, argv[1], (char*)"OTR1N2E3");
	checkPlate(argc, argv[1], (char*)"AM1E2D3");
	checkPlate(argc, argv[1], (char*)"SHR5I3I3");
