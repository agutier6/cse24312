/**********************************************
* File: GoogleStack.cpp
* Author: Matthew Morrison
* Email: matt.morrison@nd.edu
* 
* This is a solution to the Google License Interview question 
* using std::stack to store the dictionary words
* 
* Compilation command: g++ -g -std=c++11 -Wpedantic GoogleStack.cpp -o GoogleStack
* Run: ./GoogleStack [DictionaryFile] [License]
* Example: ./GoogleStack words.txt RT011OS
...
*
* Comparison to forward_list - Give the same order and answer 
...
* Pre-conditions : std::ifstream& dictionaryFile, std::stack< std::string >& dictionaryStack
* Post-conditions: none
*  
* This function reads all the dictionary words, and stores them in a STL Stack 
********************************************/
void getDictionaryWords(std::ifstream& dictionaryFile, std::stack< std::string >& dictionaryStack){
	// Get the strings and put in the Stack
	std::string wordIn;
	while (dictionaryFile >> wordIn)
	{
		dictionaryStack.push(wordIn);
	}
}
...
void checkPlate(int argc, char* fileName, char* plateStr){
	...
	// Create a Stack of dictionary words 
	std::stack< std::string > dictionaryStack;
	...
	// Get the strings and put in the Stack
	getDictionaryWords(dictionaryFile, dictionaryStack);
	...
	while(!dictionaryStack.empty()){
		...
		// Do not hash dictionaryStack.top() if final Word is the same length - Save run time 
		// Must also account for if no word is found yet (finalWord.length() > 0)
		if(dictionaryStack.top().length() < finalWord.length() || finalWord.length() == 0){
		
			// Check the word, and update final word if it qualifies
			HashWord(HashPlate, dictionaryStack.top(), finalWord);
